# Team Work Division - Balanced Distribution

## (Member 1) - Total: ~253 lines
**Algorithms (2):**
- `src/algorithms/kmp_algorithm.c` (143 lines)
- `src/algorithms/rabin_karp_algorithm.c` (110 lines)
...
## (Member 2) - Total: ~468 lines
**Algorithm (1):**
- `src/algorithms/boyer_moore_algorithm.c` (169 lines - Complex)

**Utilities (2):**
- `src/utils/utils.c` (179 lines)
- `src/utils/dna_sequence_handler.c` (120 lines)

---

## (Member 3) - Total: ~350 lines
**Algorithms (2):**
- `src/algorithms/shift_or_algorithm.c` (215 lines - Most complex)
- `src/algorithms/z_algorithm.c` (135 lines)

---

## (Member 4) - Total: ~323 lines
**Algorithms (2):**
- `src/algorithms/aho_corasick_algorithm.c` (198 lines - Very complex)
- `src/algorithms/suffix_tree.c` (125 lines - Complex data structure)

---

## (Member 5) - Total: ~376 lines
**Algorithm (1):**
- `src/algorithms/levenshtein_algorithm.c` (117 lines)

**Infrastructure:**
- `bench/benchmark_runner.py` (91 lines - Performance testing)
- `bench/python_regex_bench.py` (35 lines - Python comparison)

---

## Shared Responsibility
- `src/main.c` (709 lines) - **All members integrate their algorithms into the main menu**
- `include/pattern_matching.h` (133 lines - All function declarations)
...
### **Problem:** "./bin/dna_pattern_matching: No such file or directory"
**Fix:** You forgot to compile! Run:
```bash
make
```

### **Problem:** "Error opening file" when loading FASTA
**Fix:** The file doesn't exist. Generate it:
```bash
make sample
```

### **Problem:** "Please load a sequence first!"
**Fix:** You tried to search without data. Press `1` or `2` first!

### **Problem:** Python script error: "ModuleNotFoundError: No module named 'matplotlib'"
**Fix:**
```bash
pip install matplotlib
```

### **Problem:** Graph looks weird or doesn't show all algorithms
**Fix:** Make sure you compiled first (`make`), then run the benchmark

---

## 📊 Understanding Your Results

### **When you search for a pattern:**
```
✅ Found 8 matches          ← How many times the pattern appears
⏱️ Time taken: 0.002 ms    ← How fast it was (lower = better)
📍 Match positions: 0, 4, 8 ← Exact locations in the DNA
```

### **When you compare algorithms (Option 8):**
- Look at the "Time (ms)" column
- **Smallest number = Fastest algorithm**
- All should find the same number of matches

### **In the benchmark graph:**
- **X-axis:** DNA sequence length (bigger = harder test)
- **Y-axis:** Time in milliseconds (lower = faster)
- **Best algorithm:** Line closest to the bottom
- **Typical winner:** Suffix Tree (green) or Boyer-Moore (orange)

---

## 🏆 WHAT YOU CAN SAY ABOUT YOUR PROJECT

**When someone asks "What does your project do?":**

*"It's a DNA pattern search engine that implements 8 different algorithms in C. You can search for genetic patterns, compare which algorithm is fastest, and generate performance graphs. It handles exact matching, fuzzy matching, and multi-pattern searches."*

**When asked "How does it work?":**

*"You compile it with `make`, run it, load a DNA sequence file, type a pattern like 'ACGT', and it finds all occurrences instantly. You can race all 8 algorithms against each other or generate benchmark graphs to see performance differences."*

**When asked "What's special about it?":**

*"It's 10-100x faster than Python because it's written in pure C. The interactive menu makes it easy to use, and the benchmarking tools let you analyze algorithm performance scientifically with visual graphs."*
...
We have implemented 8 algorithms. **You don't need to understand how they work** - just know what they're good for:

### **Fast Exact Searchers:** 🎯
1. **KMP** - Reliable, never backtracks (Option 3)
2. **Boyer-Moore** - Usually the fastest for long patterns (Option 4)
3. **Suffix Tree** - Instant search after building a tree (Option 5)
4. **Shift-Or** - Lightning fast for short patterns using bit operations (Option 6)
5. **Rabin-Karp** - Uses math/hashing (Option 11)
6. **Z-Algorithm** - Good at finding repeating patterns (Option 12)

### **Special Purpose:** 🌟
7. **Levenshtein** - Finds similar patterns (typos/mutations) (Option 7)
8. **Aho-Corasick** - Searches for many patterns at once (Option 13)

**Pro Tip:** Just run Option 8 to see which is fastest for your data!
...
├── src/
│   ├── main.c                  ← Menu system
│   ├── algorithms/             ← 8 algorithm files (the brains)
